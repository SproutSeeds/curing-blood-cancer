...
| `clinicaltrials_gov` | U.S. National Library of Medicine | trial registry | all registered blood-cancer studies | `trial-search` | Trial listings do not prove availability, eligibility, safety, or efficacy. |
| `clinicaltrials_gov_api_v2` | U.S. National Library of Medicine | trial registry API | all registered blood-cancer studies | `trial-search`, `data-reuse` | API output is a registry snapshot; trial details, status, eligibility, and site availability still require verification. |
...
| `ncbi_geo` | National Center for Biotechnology Information | functional genomics data repository | public functional genomics datasets, blood-cancer studies when deposited | `dataset-discovery`, `data-reuse`, `literature-search` | Submitted datasets vary in consent, reuse terms, metadata completeness, and study quality; dataset presence does not establish clinical truth. |
| `nci_gdc_data_portal` | National Cancer Institute Genomic Data Commons | cancer genomics data portal and API | NCI cancer genomics programs, harmonized cancer genomics datasets | `dataset-discovery`, `genomics-data`, `data-reuse` | Some data require controlled access; cohort inclusion and harmonization do not establish patient-specific interpretation or clinical actionability. |
...
| `pubmed_cui_2024_mrd_clonal_evolution` | PubMed-indexed literature | open-access research article | multiple myeloma, measurable residual disease, clonal evolution, therapy resistance, single-cell RNA sequencing | `literature-search`, `mechanism-map`, `geometry-pilot`, `mrd-resistance-context` | Single-cell study context; use for source-specific mechanism extraction and research mapping, not patient-specific MRD interpretation, treatment selection, relapse prediction, or cure claims. |
| `pubmed_li_2025_single_cell_myeloma_review` | PubMed-indexed literature | review article | multiple myeloma, single-cell analysis, clonal evolution, tumor microenvironment, therapy evasion | `literature-search`, `mechanism-map`, `geometry-pilot`, `single-cell-context` | Review article; use for context and unresolved translation barriers, not clinical implementation claims, treatment guidance, or patient-specific interpretation. |
| `pubmed_samur_2023_single_cell_profiling_myeloma` | PubMed-indexed literature / Blood | open-access review article | multiple myeloma, single-cell profiling, single-cell sequencing, clinical translation, implementation challenges | `literature-search`, `mechanism-map`, `geometry-falsification`, `single-cell-translation-context` | Review article; use for single-cell profiling problems and translation-context extraction only, not clinical implementation claims, test-ordering guidance, treatment selection, or patient-specific interpretation. |
...
| `pubmed_besliu_2025_evolving_landscape_myeloma` | PubMed-indexed literature | review article | multiple myeloma, risk stratification, T-cell-directed therapy, BCMA, GPRC5D, relapsed or refractory multiple myeloma | `literature-search`, `mechanism-map`, `geometry-pilot`, `therapy-pressure-context` | Review article; use for treatment-landscape context and open questions, not product comparison, sequencing advice, trial recommendation, or patient-specific treatment fit. |
...
| `pmc_decide_ai_2022_early_clinical_eval` | DECIDE-AI Expert Group / Nature Medicine | reporting guideline | clinical decision support, artificial intelligence, early clinical evaluation, human factors, dataset shift | `literature-search`, `model-governance`, `model-validation`, `machine-representation` | Early clinical evaluation reporting guideline; use for future live-evaluation gate language only, not authorization for live use, clinical trial operation, deployment, or patient-specific decision support. |
| `fda_imdrf_gmlp_guiding_principles_2025` | U.S. Food and Drug Administration / IMDRF | AI/ML medical device guiding principles | AI/ML medical devices, machine learning, model lifecycle, digital health | `model-governance`, `model-validation`, `machine-representation` | General AI/ML medical-device lifecycle guidance; not myeloma-specific, not regulatory clearance, not model validation, not deployment authorization, and not clinical advice. |
| `pubmed_probast_2019_prediction_model_risk_of_bias` | PROBAST Group / Annals of Internal Medicine | risk-of-bias assessment tool | clinical prediction models, risk of bias, applicability, model appraisal | `literature-search`, `model-validation`, `model-governance`, `machine-representation` | Prediction-model appraisal tool; does not validate any model, rank evidence, establish clinical utility, authorize deployment, or provide patient-specific prediction. |
...
| `pubmed_cui_2024_mrd_clonal_evolution` | https://pubmed.ncbi.nlm.nih.gov/38900040/ | 2026-04-19 |
| `pubmed_li_2025_single_cell_myeloma_review` | https://pubmed.ncbi.nlm.nih.gov/40002248/ | 2026-04-19 |
| `pubmed_samur_2023_single_cell_profiling_myeloma` | https://pubmed.ncbi.nlm.nih.gov/37196627/ | 2026-04-20 |
| `pubmed_bolli_2014_genomic_heterogeneity_myeloma` | https://pubmed.ncbi.nlm.nih.gov/24429703/ | 2026-04-20 |
| `pubmed_besliu_2025_evolving_landscape_myeloma` | https://pubmed.ncbi.nlm.nih.gov/39941892/ | 2026-04-19 |
...
| `pmc_decide_ai_2022_early_clinical_eval` | https://pmc.ncbi.nlm.nih.gov/articles/PMC9116198/ | 2026-04-20 |
| `fda_imdrf_gmlp_guiding_principles_2025` | https://www.fda.gov/medical-devices/software-medical-device-samd/good-machine-learning-practice-medical-device-development-guiding-principles | 2026-04-20 |
| `pubmed_probast_2019_prediction_model_risk_of_bias` | https://pubmed.ncbi.nlm.nih.gov/30596875/ | 2026-04-20 |
