...
- [x] `orp-frontier-gap-sweep-mode-v0` ORP Frontier Gap Sweep Mode v0 (`complete`)
  - goal: Create a reusable ORP mode and first multiple myeloma sweep for deep direction checks, stale-state drift detection, public-source blind-spot discovery, and next public-safe queue synthesis.
...
- [ ] `machine-representation-expert-validation-human-authorization-blocker-v0` Machine Representation Expert Validation Human Authorization Blocker v0 (`blocked`)
  - goal: Record that no outreach, public issue operation, response intake, expert-review claim upgrade, or external validation execution is authorized after the no-outreach prep packet, internal public-source extraction pass, and frontier gap sweep mode; the next autonomous step must be a newly named no-outreach public-source extraction phase or another public-safe status-prep item.
