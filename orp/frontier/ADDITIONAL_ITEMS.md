...
- [x] `machine-representation-source-gap-internal-extraction-v0` Machine Representation Source-Gap Internal Extraction v0 (`complete`)
  - goal: Complete a no-outreach, public-source-only extraction pass for machine-representation source-gap pressure points, mapping therapy-exposure context and validation-governance anchors while preserving the fusion-architecture source-context-needed blocker.
- [x] `orp-frontier-gap-sweep-mode-v0` ORP Frontier Gap Sweep Mode v0 (`complete`)
  - goal: Add a reusable ORP mode and first multiple myeloma sweep for direction checks, stale-state drift, current-source blind spots, and next public-safe queue synthesis without outreach or clinical claims.
- [x] `residual-disease-modality-discordance-source-extraction-v0` Residual Disease Modality Discordance Source Extraction v0 (`complete`)
...
- [x] `measurement-refusal-wrapper-integration-dry-run-v0` Measurement Refusal Wrapper Integration Dry Run v0 (`complete`)
  - goal: Complete a no-outreach, synthetic-only dry run that maps refused measurement outputs into model-output boundary wrapper metadata before wrapper state-machine transition checking or model-output reuse.
...
- [ ] `machine-representation-expert-validation-human-authorization-blocker-v0` Machine Representation Expert Validation Human Authorization Blocker v0 (`blocked`)
  - goal: Block actual external expert-validation execution, outreach, private correspondence, issue operations, response intake, expert-review claim upgrades, real quality review, and clinical interpretation unless a human explicitly authorizes scope or selects a new named no-outreach public-source phase.
