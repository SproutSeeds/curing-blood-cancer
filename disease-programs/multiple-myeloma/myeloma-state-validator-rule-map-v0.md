...
| `msv_24_measurement_refusal_wrapper_state_machine_negative_safety_fixtures_visible` | `measurement-refusal-wrapper-state-machine-negative-safety-fixtures-v0`, `measurement-refusal-wrapper-state-machine-v0`, `measurement-refusal-wrapper-negative-safety-fixtures-v0`, and `model-output-boundary-wrapper-v0` | `mob_12_assay_specimen_quality_refusal` | `fixture_set_id`, `target_checker_id`, `target_state_machine_id`, `negative_fixtures`, `mutations`, `expected_failed_check_ids`, `data_boundary`, `handoff` | `measurement-refusal-wrapper-state-machine-negative-safety-fixtures-v0` | Seventeen synthetic negative fixtures fail closed against expected wrapper state-machine checks. | Preserve unsafe-transition fixture metadata only; reject boundary poisoning, missing clinical boundaries, state drift, transition drift, trace diversion, blocked-rule gaps, missing invariants, forbidden output fields, and human-gate drift. | `endpoint_interpretation_blocked`, `prognosis_or_monitoring_blocked`, `treatment_guidance_blocked`, `matching_or_ranking_blocked`, `decision_authorization_blocked` | `wrapper_state_machine_falsification_audit` |
...
| `measurement-refusal-wrapper-state-machine-negative-safety-fixtures-v0` | `msv_10` through `msv_24`. | Seventeen state-machine negative fixtures mutate transition-surface safety and must fail closed for boundary poisoning, clinical boundary gaps, state coverage drift, transition drift, trace diversion, blocked-rule gaps, missing invariants, forbidden output fields, and human-gate drift. |
...
The later [Measurement Refusal Wrapper State Machine Negative Safety Fixtures v0](measurements/measurement-refusal-wrapper-state-machine-negative-safety-fixtures-v0.md)
and [`measurement-refusal-wrapper-state-machine-negative-safety-fixture-check-v0`](../../tools/check_measurement_refusal_wrapper_state_machine_negative_safety_fixtures.py)
close `msv_24` by proving the wrapper state-machine surface fails closed on
seventeen synthetic unsafe transition mutations. The fixture pack carries no
real case data, model output, clinical interpretation, ranking, real-report
review, publication authorization, or patient-specific decision content.
