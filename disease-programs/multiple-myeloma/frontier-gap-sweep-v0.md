...
| `fgs_04_assay_and_specimen_quality` | Sample quality, hemodilution, missingness, failed assays, sensitivity, threshold, timing, and method lineage. | The machine stack needs refusal reasons for bad or incomplete measurement context before it needs better prediction heads. | Completed as `assay-specimen-quality-failure-mode-checklist-v0`; next pressure is `measurement-state-refusal-fixture-extension-v0`. | No lab interpretation or test-ordering advice. |
...
1. `measurement-state-refusal-fixture-extension-v0`
...
assay/specimen quality successor are now complete. The top remaining
recommendation is `measurement-state-refusal-fixture-extension-v0`, because the
new failure modes should be pressure-tested before any executable validator or
model-output path can reuse them.
