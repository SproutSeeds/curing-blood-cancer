...
1. `measurement-refusal-validator-skeleton-v0`
...
`measurement-refusal-output-route-table-v0` are also complete. The top
remaining recommendation is `measurement-refusal-validator-skeleton-v0`,
because the checked refusal output records now have deterministic routing and
need a synthetic-only executable skeleton before any model-output path can
reference them.
