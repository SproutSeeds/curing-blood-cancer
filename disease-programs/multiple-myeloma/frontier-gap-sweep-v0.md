...
1. `measurement-refusal-wrapper-state-machine-falsification-audit-v0`
...
structural report over routed refusal records. The later
`measurement-refusal-negative-safety-fixtures-v0` is complete as adversarial
fail-closed coverage for missing routes, duplicate routes, unsafe destination
contracts, unsafe route families, forbidden clinical fields, forbidden ranking
fields, and public processing of private quality-review requests. The later
`measurement-refusal-wrapper-integration-dry-run-v0` is complete as
wrapper-metadata-only mapping for refused outputs, and the later
`measurement-refusal-wrapper-negative-safety-fixtures-v0` is complete as
adversarial fail-closed coverage for unsafe wrapper mutations. The later
`measurement-refusal-wrapper-state-machine-v0` is complete as explicit
wrapper-state transition metadata. The top remaining recommendation is now
`measurement-refusal-wrapper-state-machine-falsification-audit-v0`,
because the allowed wrapper states and transitions should be adversarially
pressure-tested before any model-output reuse.
