...
| `mse_06_measurement_timeline_and_mrd` | Measurement timeline fields should preserve response, MRD, relapse, lab, imaging, residual-disease modality family, method, threshold, sample/specimen, timepoint, durability, endpoint role, source context, discordance state, and limitation state. | `fda_mrd_cr_endpoint_guidance_2026`; `pubmed_kubicki_2025_blood_ms_mrd`; `eclinicalmedicine_2026_mrd_petct_concordance`; `pubmed_yip_2025_spatial_transcriptomics_myeloma`; local anchors: `measurement-normalization-contract-v0`, `mrd-and-relapse-measurement-glossary-v0`, `mrd-endpoint-language-guardrail-addendum-v0`, `residual-disease-modality-discordance-source-extraction-v0` | `source_id_mapped` | FDA source is draft and nonbinding; residual-disease modality extraction adds blood MS, PET-CT discordance, and spatial context anchors, but no modality can be reused as cure, prognosis, monitoring, treatment, response, or global disease-state interpretation. | cure claim, endpoint interpretation, prognosis, monitoring guidance, treatment guidance, trial guidance, modality ranking, patient-specific residual-disease interpretation | Extract endpoint-role, method, threshold, sample, timepoint, paired-modality state, assay/specimen quality, and discordance fields into source-scoped measurement tasks. |
...
| `mse_10_prediction_head_boundaries` | Progression, response, MRD, resistance, and residual-disease modality-discordance states may be named only as research-output placeholders with uncertainty, calibration, missingness, and blocked-use state. | local anchors: `model-output-boundary-wrapper-v0`, `myeloma-state-validator-rule-map-v0`, `evidence-retrieval-packet-v0`, `residual-disease-modality-discordance-source-extraction-v0` | `boundary_only` | Boundary is mapped, but no prediction head, modality-discordance interpreter, validation, calibration, or clinical output is implemented. | progression risk, response probability, MRD interpretation, residual-disease modality interpretation, resistance attribution, recommendation, ranking | Draft expert-review questions about whether the refusal wrapper, modality-discordance states, and blocked-use labels are complete enough for future synthetic-only outputs. |
...
| `mr_source_task_04_endpoint_fields` | `mse_06` | Extract MRD, response endpoint, and residual-disease modality-discordance field requirements from public measurement sources and guardrails. | Method, threshold, sample/specimen, timepoint, durability, endpoint role, paired-modality state, assay/specimen quality, discordance state, and limitation fields are explicit. |
...
The later [Residual Disease Modality Discordance Source Extraction v0](measurements/residual-disease-modality-discordance-source-extraction-v0.md) adds first-class source anchors for blood mass spectrometry MRD, MRD/PET-CT discordance, spatial marrow architecture, assay/specimen quality, and output-wrapper refusal states. This improves `mse_06` and `mse_10` without changing the core boundary: no residual-disease modality context can become patient-specific interpretation, prognosis, monitoring, treatment guidance, modality ranking, model validation, or a cure claim.
