...
| `mse_05_treatment_exposure_context` | Treatment exposure history should be represented by therapy class, line or timing bucket, response linkage, refractory context, toxicity or constraint category, and source/review status. | local anchors: `therapy-exposure-timeline-contract-v0`, `multiple-myeloma-treatment-class-taxonomy-v0`, `immune-therapy-sequencing-access-boundary-v0`; `source_context_needed` | `source_context_needed` | The public shape exists, but this row still needs source extraction for therapy-exposure vocabulary and cohort context beyond local contracts. | sequencing, eligibility, availability, access guidance, treatment advice, trial advice, ranking | Open a public source task for therapy-exposure vocabulary, class labels, response linkage, and limitation context. |
...
| `mse_11_training_and_validation_standard` | Future validation should consider site-aware, calendar-time, newly diagnosed versus relapsed/refractory separation, calibration, missing-modality robustness, comparator features, subgroup review, and dataset-shift warnings. | local anchor: `multiple-myeloma-machine-representation-stack-v0`; `source_context_needed` | `source_context_needed` | These validation requirements are safety requirements, not completed benchmark evidence; public method sources and model-governance review are still needed. | clinical utility claim, model validation claim, deployment readiness, publication authorization | Create a source-gap task for public validation-method sources and model-governance review questions. |
...
| `mr_source_task_03_therapy_exposure_sources` | `mse_05` | Identify public sources for therapy-exposure vocabulary, response linkage, refractory context, and limitation fields. | Source IDs are added or the gap remains `source_context_needed`. |
...
| `mr_source_task_06_model_method_sources` | `mse_09`, `mse_11` | Find public method or governance sources for fusion architecture and validation standards, or keep them expert-review-needed. | Method rows are source-scoped and cannot be read as implementation approval. |
...
biological and measurement anchors, but some modeling and validation claims are
still architecture proposals rather than source-extracted evidence. The
highest-value next unit is a contribution-ready source-gap task queue so
contributors can resolve `source_context_needed` rows without creating model
code, evidence ranking, clinical interpretation, or patient-specific outputs.
...
packet, and implementation completion audit are complete, and the next active
state should be `machine-representation-public-scope-human-gate-blocker-v0`.
