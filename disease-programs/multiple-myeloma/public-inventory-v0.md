...
| Class | Count | Meaning |
| --- | ---: | --- |
| dataset | 20 | Catalogs, public task queues, evidence-gap records, measurement context, issue indexes, response ledgers, outreach maps, source registry, source extraction, source-gap task routing, and synthetic examples. |
...
| --- | --- |
| Is the public v0 artifact surface complete for the current autonomous loop? | The earlier public loop handoff, frontier lanes, adaptive case-to-cure master backlog, static frontend phase, state-object schema, synthetic state fixture, model-output wrapper, validator rule map, source extraction table, source-gap task queue, source-gap issue draft packet, and machine-representation implementation completion audit are complete for the current named phase. |
| Are there ready public tasks left in the current queues? | No autonomous machine-representation implementation item remains after the completion audit. The active next state is `machine-representation-public-scope-human-gate-blocker-v0`, which requires human selection of a new named phase or clearance of a named expert, private-lab, clinical, model-governance, legal, regulatory, publication, or human-review gate. |
...
| What is the live adaptive phase now? | `myeloma-machine-representation-implementation-v0`, anchored by [Case-To-Cure Adaptive Master Plan v0](case-to-cure-adaptive-master-plan-v0.md), with ORP active item `machine-representation-public-scope-human-gate-blocker-v0` after the completion audit closed the current implementation artifacts. |
