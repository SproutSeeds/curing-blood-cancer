# Schemas

Structured data contracts live here.

Schemas should make public artifacts easier to validate, compare, and reuse
without weakening the repo's safety boundaries.

## Current Schemas

- [Artifact Metadata Schema](./artifact-metadata.schema.json)
- [Artifact Catalog Schema](./artifact-catalog.schema.json)
- [Case-Feature Bundle Public Summary v0](./case-feature-bundle-public-summary-v0.md)
- [Case-To-Cure Synthetic Pipeline Schema](./case-to-cure-synthetic-pipeline.schema.json)
- [Claim Set Schema](./claim-set.schema.json)
- [Disease Map Schema v0](./disease-map-schema-v0.md)
- [Evidence Claim Schema v0](./evidence-claim.schema.json)
- [Evidence Gap Register Schema](./evidence-gap-register.schema.json)
- [Mechanism Extraction Schema](./mechanism-extraction.schema.json)
- [Mechanism Map Schema](./mechanism-map.schema.json)
- [Measurement Context Audit Schema](./measurement-context-audit.schema.json)
- [Measurement Glossary Schema](./measurement-glossary.schema.json)
- [Myeloma State Object Schema v0](./myeloma-state-object-schema-v0.md)
- [Opportunity Map Schema](./opportunity-map.schema.json)
- [Public Task Queue Schema](./public-task-queue.schema.json)
- [Target Record Schema v0](./target-record-schema-v0.md)
- [Therapy Record Schema v0](./therapy-record-schema-v0.md)
- [Trial-Landscape Record Schema v0](./trial-landscape-record-schema-v0.md)
- [Open-Question Record Schema v0](./open-question-record-schema-v0.md)
- [Review-Packet Builder Manifest Schema v0](./review-packet-builder-manifest-schema-v0.md)
- [Review-Packet Route-Table Output Schema v0](./review-packet-route-table-output-schema-v0.md)
- [Measurement Refusal Output Schema v0](./measurement-refusal-output-schema-v0.md)

## Templates And Examples

- [Evidence Claim Template v0](./evidence-claim-template-v0.json)
- [Evidence Claim Example v0](./evidence-claim-example-v0.json)
- [Disease Map Template v0](./disease-map-template-v0.json)
- [Target Record Template v0](./target-record-template-v0.json)
- [Therapy Record Template v0](./therapy-record-template-v0.json)
- [Trial-Landscape Record Template v0](./trial-landscape-record-template-v0.json)
- [Open-Question Record Template v0](./open-question-record-template-v0.json)
- [Review-Packet Builder Manifest Template v0](./review-packet-builder-manifest-template-v0.json)
